...
export default class FavoritesDAO {
    static async injectDB(conn) {
        ...
        try {
            const mongo = await conn.db(process.env.MOVIEREVIEWS_COLLECTION)
            favoritesCollection = mongo.collection("favorites");
            moviesCollection = mongo.collection("movies");
        }
        catch (e) {
    ...
    static async getFavorites(id) {
        ...
        try {
            ...
            console.log("favorites", favorites) // testing
            return favorites[0];
        } catch (e) {
